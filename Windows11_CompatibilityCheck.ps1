...
#Get processor name and split it
$ProcessorName = (Get-CimInstance -ClassName Win32_Processor).Name
$ProcessorWords = $ProcessorName -split ' '

# Match against known models
$ProcMatch = $ProcessorWords | ForEach-Object {
    $match = $models | Where-Object { $_.model -eq $_ }
    if ($match) { return $match.compatible }
}
$ProcessorCompatible = $ProcMatch -contains $true


# Build results
$Results = [PSCustomObject]@{
    "TPM Compatible"       = [bool](Get-Tpm).tpmpresent
    "Processor Compatible" = [bool]$ProcessorCompatible
